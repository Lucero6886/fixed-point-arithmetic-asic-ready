#!/usr/bin/env python3
"""
Extract Yosys summary for Project 8.2.1:
SC Decoder N=16 reference RTL synthesis baseline.
"""
...
LOG_PATH = Path("synth/reports/sc_decoder_n16_ref_flat_yosys.log")
CSV_PATH = Path("results/summary/sc_decoder_n16_ref_yosys_summary.csv")
MD_PATH = Path("results/summary/sc_decoder_n16_ref_yosys_summary.md")


def parse_yosys_log(text: str) -> dict:
    blocks = re.findall(
        r"=== sc_decoder_n16_ref ===(?P<body>.*?)(?=\n=== |\nEnd of script\.|\Z)",
        text,
        flags=re.S,
    )

    if not blocks:
        raise RuntimeError("Could not find '=== sc_decoder_n16_ref ===' block in Yosys log.")
...
def write_markdown(summary: dict) -> None:
    ...
    with MD_PATH.open("w", encoding="utf-8") as f:
        f.write("# SC Decoder N=16 Reference RTL Yosys Summary\n\n")
        ...
        f.write("\n## 3. Interpretation\n\n")
        f.write(
            "This synthesis result is the reference N=16 RTL baseline. "
            "It is intended for correctness and future architecture comparison, "
            "not as an optimized resource-shared implementation.\n\n"
        )
        f.write(
            "The key number to compare later is the total cell count and estimated "
            "combinational cell count against the future resource-shared N=16 decoder.\n"
        )
